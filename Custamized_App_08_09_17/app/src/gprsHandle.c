...
//Globals
U8		gprsAttached,gotPPPip,socketConnected,socConSuccess104;
...
char *jsonMsg = "{\"IMEI\":\"%lf\"LAT\":\"%lf\",\"LON\":\"%lf\"}"; 
...
int sockCreate(char *ipStr,UINT16 port)
{
	...
	while(retry)
	{
		...
		for(i=0;i<10;i++)
		{
			...
			if(ret < 0)
			{
				printf1("\rConnect TCP server failed. Return value: %d\n",ret); 
				delayInms(3000);
				continue;
			}
			else
		...
		if(i >= 10)
		{
			printf1("\rConnect TCP server failed. Tries: %d\n",retry);
			closeSocket(sockTcp);
			retry --;
			continue;
		}
...
int sendPacket(const char* buf, unsigned int count)
{
	int	bytesSent=0;
...
int postGpsInfo(void)
{
	...
	//socket create
	if(sockCreate(ipStr1, port)<0) 
	{
		printf1("Could not create socket\n"); 
	}
	else 
	{
		len=sendPacket(jsonMsg, strlen(jsonMsg)); 
		if(len<0) 
		{
			printf1("Data send failed\n"); 
		}
		else 
		{
			printf1("%d length of data sent.\n"); 
		}
